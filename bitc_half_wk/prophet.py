...
for c, i in df.iterrows():
    ...
    delta4 = abs(pd.to_datetime(i['日期']) - pd.to_datetime('2025-06-11')).days
...
# 按照日期对数据集进行拆分
split_data = df[df['日期'] < pd.to_datetime('2024-03-01')]
split_date = pd.to_datetime('2023-11-01')  # 选择拆分日期
train_data = split_data[split_data['日期'] < split_date]
test_data = split_data[split_data['日期'] >= split_date]
...
# 绘制实际涨跌幅和预测结果的对比图
plt.figure(figsize=(12, 6))
plt.plot(Y, Y_test, label='Actual Returns')
plt.plot(Y, test_predictions, label='Predicted Returns')
plt.title('Actual vs Predicted Returns')
plt.xlabel('日期')
plt.ylabel('收盘')
plt.legend()
